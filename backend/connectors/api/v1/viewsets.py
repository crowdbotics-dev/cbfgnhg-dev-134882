...
class spr65newcinaViewSet(BaseConnectorViewSet):
    TOKEN = settings.CONNECTOR_SPR65NEWCINA_TOKEN
    BASE_URL = "https://api.newcinnamon.com"
    AUTH_TYPE = "bearer"
    IDENTIFIER = "CONNECTOR_SPR65NEWCINA"

    @action(detail=False, methods=["get"], url_path="test/search/new")
    def test_search_new(self, request, *args, **kwargs):
        params = request.query_params
        payload = request.data
        data = self._api_call(
            "/test/search/new/", request_type="get", payload=payload, params=params
        )
